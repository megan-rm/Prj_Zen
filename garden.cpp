		...
		if (line == "[WORLD_TILES]") {
			...
			while (!file.eof() && line.at(0) != '[') {
				...
				while (std::getline(s_stream, line, '|')) {
					...
					world.at(x).at(y).set_tile_id(tile_id);
					world.at(x).at(y).set_permeability(permeability);
					world.at(x).at(y).set_max_saturation(max_saturation);
					world.at(x).at(y).set_saturation(saturation);
